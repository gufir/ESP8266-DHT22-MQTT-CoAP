...
//ADD POST non Confirmable MSG
uint16_t Coap::post_noncon(IPAddress ip, int port, const char *url, const char *payload) {
	return this->send(ip, port, url, COAP_NONCON, COAP_POST, NULL, 0, (uint8_t *)payload, strlen(payload));
}

uint16_t Coap::post_noncon(IPAddress ip, int port, const char *url, const char *payload,size_t payloadlen) {
